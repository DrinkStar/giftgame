...
### 补丁（2026-08-14 四次 grilling：执行清单遗漏）

共 17 问。写入 `strategy.md` §5/§6/§9。不改游戏代码。

| 锁定 | 内容 |
|---|---|
| T8.5.0 | 合海进 `Game.tscn` 为 8.5 第一项；`ocean_test` 仍实验室；不提前到教程前 |
| T8p.6 | 手持火把照明；**仍可弱近战** |
| T8p.2 | 木头 **和** 椰子都做 |
| T8.4 | Iter 8 最小 CraftUI（C 键） |
| T8.3 | 教程日长 20–30 真实分钟 |
| T8p.4 | 跳夜时体力回满；日常仍被动回复 |
| T8.4b | CraftUI 之后才拦 `RequiresCooking`；T8p.1 不拦 |
| T8.1 | F5/F9/启动自动读走 `SaveService` 总控，内部复用建筑 JSON |
| T8.5.8 | 储物箱独立库存 + 进档 |
| R2 | `MaxDurability` 预留；首版不扣不坏 |
| T8.5.5 | 夜间敌人伤害/移速乘数 |
| T8.5.9 | `StoryInteractable` 绑 `ch2.ruin` |
| P1.5 | 首版热键冒充双武器槽，不排任务 |
| T8.5.4 | 投矛不消耗（跟铁矛一起） |
| T8.5.10/11 | 第二章强制：种田+遗迹；第三章强制：木筏；Boss 不进强制教程 |

仍不单开：`Contains` 武器匹配、蓝图 ID 对不齐（berry/berries 等）。
